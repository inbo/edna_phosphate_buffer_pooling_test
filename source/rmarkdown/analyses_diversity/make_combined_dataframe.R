...
load(file = file.path(file_directory_bioinformatica, "18S/data/physeq_18S_mbag_annelida_metadata17.Rdata"))

# Make diversity table for 18S Annelida (full)
table_18S_annelida <- make_diversity_index_table_from_phyloseq_obj(
  phyloseq_obj = physeq_18S_annelida,
  taxon_name = "annelida", unit_value="ASV", primerset = "18S_universal",
...
# Load in the non-rarefied dataframe
load(file = file.path(file_directory_bioinformatica, "18S/data/physeq_18S_mbag_collembola_metadata17.Rdata"))

# Make diversity table for 18S Collembola (full)
table_18S_collembola <- make_diversity_index_table_from_phyloseq_obj(
  phyloseq_obj = physeq_18S_collembola,
  taxon_name = "collembola", unit_value="ASV", primerset = "18S_universal",
  metadata_df = metadata,
  micro_or_macro_organism = "MACRO",
  list_runs_with_unrelated_samples = c("25002"))
...
# Load in the non-rarefied dataframe
load(file = file.path(file_directory_bioinformatica, "18S/data/physeq_18S_mbag_arthropoda_metadata17.Rdata"))

# Make diversity table for 18S Protisten (full)
table_18S_arthropoda <- make_diversity_index_table_from_phyloseq_obj(
  phyloseq_obj = physeq_18S_arthropoda,
  taxon_name = "arthropoda", unit_value="ASV", primerset = "18S_universal",
  metadata_df = metadata,
  micro_or_macro_organism = "MACRO",
  list_runs_with_unrelated_samples = c("25002"))
...
# Load in the non-rarefied dataframe
load(file = file.path(file_directory_bioinformatica, "18S/data/physeq_18S_mbag_fungi_metadata17.Rdata"))

# Make diversity table for 18S Fungi (full)
table_18S_fungi <- make_diversity_index_table_from_phyloseq_obj(
  phyloseq_obj = physeq_18S_fungi,
  taxon_name = "fungi", unit_value="ASV", primerset = "18S_universal",
  metadata_df = metadata,
  micro_or_macro_organism = "MACRO",
  list_runs_with_unrelated_samples = c("25002"))
...
# Load in the non-rarefied dataframe
load(file = file.path(file_directory_bioinformatica, "18S/data/physeq_18S_mbag_metazoa_metadata_17.Rdata"))

# Make diversity table for 18S Metazoa (full)
table_18S_metazoa <- make_diversity_index_table_from_phyloseq_obj(
  phyloseq_obj = physeq_18S_metazoa,
  taxon_name = "metazoa", unit_value="ASV", primerset = "18S_universal",
  metadata_df = metadata,
  micro_or_macro_organism = "MACRO",
  list_runs_with_unrelated_samples = c("25002"))
...
# Load in the non-rarefied dataframe
load(file = file.path(file_directory_bioinformatica, "18S/data/physeq_18S_mbag_nematoda_metadata17.Rdata"))

# Make diversity table for 18S Nematoda (full)
table_18S_nematoda <- make_diversity_index_table_from_phyloseq_obj(
  phyloseq_obj = physeq_18S_nematoda,
  taxon_name = "nematoda", unit_value="ASV", primerset = "18S_universal",
  metadata_df = metadata,
  micro_or_macro_organism = "MACRO",
  list_runs_with_unrelated_samples = c("25002"))


# 1.7 18S Platyhelminthes

# Load in the non-rarefied dataframe
load(file = file.path(file_directory_bioinformatica, "18S/data/physeq_18S_mbag_platyhelminthes_metadata17.Rdata"))

# Make diversity table for 18S Protisten (full)
table_18S_platyhelminthes <- make_diversity_index_table_from_phyloseq_obj(
  phyloseq_obj = physeq_18S_platyhelminthes,
  taxon_name = "platyhelminthes", unit_value="ASV", primerset = "18S_universal",
  metadata_df = metadata,
  micro_or_macro_organism = "MACRO",
  list_runs_with_unrelated_samples = c("25002"))
...
# Load in the non-rarefied dataframe
# The variable name is physeq_18S_protisten1_sub
load(file = file.path(file_directory_bioinformatica, "18S/data/physeq_18S_mbag_protisten_metadata17.Rdata"))

# Make diversity table for 18S Protisten (full)
table_18S_protisten <- make_diversity_index_table_from_phyloseq_obj(
  phyloseq_obj = physeq_18S_protisten1_sub,
  taxon_name = "protisten", unit_value="ASV", primerset = "18S_universal",
  metadata_df = metadata,
  micro_or_macro_organism = "MACRO",
  list_runs_with_unrelated_samples = c("25002"))
...
# Load in the non-rarefied dataframe
load(file = file.path(file_directory_bioinformatica, "18S/data/physeq_18S_mbag_rotifera_metadata17.Rdata"))

# Make diversity table for 18S Protisten (full)
table_18S_rotifera <- make_diversity_index_table_from_phyloseq_obj(
  phyloseq_obj = physeq_18S_rotifera,
  taxon_name = "rotifera", unit_value="ASV", primerset = "18S_universal",
  metadata_df = metadata,
  micro_or_macro_organism = "MACRO",
  list_runs_with_unrelated_samples = c("25002"))
...
# Load in the non-rarefied dataframe
load(file = file.path(file_directory_bioinformatica, "18S/data/physeq_18S_mbag_tardigrada_metadata17.Rdata"))

# Make diversity table for 18S Protisten (full)
table_18S_tardigrada <- make_diversity_index_table_from_phyloseq_obj(
  phyloseq_obj = physeq_18S_tardigrada,
  taxon_name = "tardigrada", unit_value="ASV", primerset = "18S_universal",
  metadata_df = metadata,
  micro_or_macro_organism = "MACRO",
  list_runs_with_unrelated_samples = c("25002"))
...
# Combined the 18S tables vertically
combined_table_18S_INBO <- bind_rows(
  table_18S_annelida,
  table_18S_collembola,
  table_18S_arthropoda,
  table_18S_fungi,
  table_18S_metazoa,
  table_18S_nematoda,
  table_18S_platyhelminthes,
  table_18S_protisten,
  table_18S_rotifera,
  table_18S_tardigrada
)

# Print to file the 18S combined tables
write_to_csv_and_rdata(diversity_index_table = combined_table_18S_INBO,
                       output_directory = output_directory,
                       output_filename = "mbag_18S_INBO_dataframe.csv")

####
# Add total_total_count (all)

# Load in the non-rarefied dataframe full 18S
load(file = file.path(file_directory_bioinformatica, "18S/data/physeq_mbag_18S_metadata17.Rdata"))

# Get total counts per sample from the full phyloseq object
total_counts_18S_all <- data.frame(
  sample = sample_names(physeq_18S_no_singletons),
  total_total_count = sample_sums(physeq_18S_no_singletons)
)

# Merge combined diversity index table with total counts
combined_table_18S_INBO_with_total <- combined_table_18S_INBO %>%
  dplyr::left_join(total_counts_18S_all, by = "sample")

# Save the updated combined table
write_to_csv_and_rdata(diversity_index_table = combined_table_18S_INBO_with_total,
                       output_directory = output_directory,
                       output_filename = "mbag_18S_INBO_dataframe_with_total_counts.csv")
...
# Load the non-rarefied phyloseq object
# The variable name is physeq_Coll01_Collembola
load(file = file.path(file_directory_bioinformatica, "Coll01/data/physeq_Coll01_Collembola_metadata17.Rdata"))

# Create phyloseq object with only known species
# Filter away entries with empty string as marker for unknown species
physeq_Coll01_collembola_known_species <- subset_taxa(physeq_Coll01_Collembola, !grepl("_?unclassified_?|_otu\\d+", species))

# Agglomerate known species OTUs with tax_glom
physeq_Coll01_collembola_known_species_tax_glom <- tax_glom(physeq_Coll01_collembola_known_species, taxrank="species")


# Make diversity table for Coll01 Collembola (full)
table_Coll01_collembola <- make_diversity_index_table_from_phyloseq_obj(
  phyloseq_obj = physeq_Coll01_Collembola,
  taxon_name = "collembola", unit_value="OTU", primerset = "Coll01",
  metadata_df = metadata,
  micro_or_macro_organism = "MACRO",
  list_runs_with_unrelated_samples = c("25002"),
  output_directory = output_directory, output_filename = "mbag_Coll01_dataframe.csv")

# Make diversity table for Coll01 Collembola (only known species)
table_Coll01_collembola_known_species_tax_glom <- make_diversity_index_table_from_phyloseq_obj(
  phyloseq_obj = physeq_Coll01_collembola_known_species_tax_glom,
  taxon_name = "collembola_tax_glom", unit_value="OTU", primerset = "Coll01",
  metadata_df = metadata,
  micro_or_macro_organism = "MACRO",
  list_runs_with_unrelated_samples = c("25002"),
  output_directory = output_directory, output_filename = "mbag_Coll01_dataframe_known_species_tax_glom.csv")

table_Coll01_collembola_known_species <- make_diversity_index_table_from_phyloseq_obj(
  phyloseq_obj = physeq_Coll01_collembola_known_species,
  taxon_name = "collembola", unit_value="OTU", primerset = "Coll01",
  metadata_df = metadata,
  micro_or_macro_organism = "MACRO",
  list_runs_with_unrelated_samples = c("25002"),
  output_directory = output_directory, output_filename = "mbag_Coll01_dataframe_known_species.csv")

####
# Add total_total_count (all)

# Load in the non-rarefied dataframe full Coll01
load(file = file.path(file_directory_bioinformatica, "Coll01/data/physeq_Coll01_all_metadata17.Rdata"))

# Get total counts per sample from the full phyloseq object
total_counts_Coll01_all <- data.frame(
  sample = sample_names(physeq_Coll01_no_singletons),
  total_total_count = sample_sums(physeq_Coll01_no_singletons)
)

# All (with unknown species)

# Merge combined diversity index table with total counts
combined_table_Coll01_INBO_with_total <- table_Coll01_collembola %>%
  dplyr::left_join(total_counts_Coll01_all, by = "sample")

# Save the updated combined table
write_to_csv_and_rdata(diversity_index_table = combined_table_Coll01_INBO_with_total,
                       output_directory = output_directory,
                       output_filename = "mbag_Coll01_INBO_dataframe_with_total_counts.csv")

# All (only known species)
# Merge combined diversity index table with total counts
combined_table_Coll01_INBO_with_total_known_species_tax_glom <- table_Coll01_collembola_known_species_tax_glom %>%
  dplyr::left_join(total_counts_Coll01_all, by = "sample")

combined_table_Coll01_INBO_with_total_known_species <- table_Coll01_collembola_known_species %>%
  dplyr::left_join(total_counts_Coll01_all, by = "sample")

# Save the updated combined table
write_to_csv_and_rdata(diversity_index_table = combined_table_Coll01_INBO_with_total_known_species_tax_glom,
                       output_directory = output_directory,
                       output_filename = "mbag_Coll01_INBO_dataframe_with_total_counts_known_species_tax_glom.csv")

write_to_csv_and_rdata(diversity_index_table = combined_table_Coll01_INBO_with_total_known_species,
                       output_directory = output_directory,
                       output_filename = "mbag_Coll01_INBO_dataframe_with_total_counts_known_species.csv")
...
# Load the non-rarefied phyloseq object
# The variable name is physeq_olig01_annelida
load(file = file.path(file_directory_bioinformatica, "Olig01/data/physeq_Olig01_Annelida_metadata17.Rdata"))

# Create phyloseq object with only known species
# Filter away entries with _otuXXX as suffix for unknown species
physeq_Olig01_annelida_known_species <- subset_taxa(physeq_olig01_annelida, !grepl("_?unclassified_?|_otu\\d+", species))

# Agglomerate known species OTUs with tax_glom
physeq_Olig01_annelida_known_species_tax_glom <- tax_glom(physeq_Olig01_annelida_known_species, taxrank="species")


# Make diversity table for Olig01 Annelida (full)
table_Olig01_annelida <- make_diversity_index_table_from_phyloseq_obj(
  phyloseq_obj = physeq_olig01_annelida,
  taxon_name = "annelida", unit_value="OTU", primerset = "Olig01",
  metadata_df = metadata,
  micro_or_macro_organism = "MACRO",
  list_runs_with_unrelated_samples = c("25002"),
  output_directory = output_directory, output_filename = "mbag_Olig01_dataframe.csv")

# Make diversity table for Olig01 Annelida (only known species)
table_Olig01_annelida_known_species_tax_glom <- make_diversity_index_table_from_phyloseq_obj(
  phyloseq_obj = physeq_Olig01_annelida_known_species_tax_glom,
  taxon_name = "annelida_tax_glom", unit_value="OTU", primerset = "Olig01",
  metadata_df = metadata,
  micro_or_macro_organism = "MACRO",
  list_runs_with_unrelated_samples = c("25002"),
  output_directory = output_directory, output_filename = "mbag_Olig01_dataframe_known_species_tax_glom.csv")

table_Olig01_annelida_known_species <- make_diversity_index_table_from_phyloseq_obj(
  phyloseq_obj = physeq_Olig01_annelida_known_species,
  taxon_name = "annelida", unit_value="OTU", primerset = "Olig01",
  metadata_df = metadata,
  micro_or_macro_organism = "MACRO",
  list_runs_with_unrelated_samples = c("25002"),
  output_directory = output_directory, output_filename = "mbag_Olig01_dataframe_known_species.csv")


####
# Add total_total_count (all)

# Load in the non-rarefied dataframe full Olig01
load(file = file.path(file_directory_bioinformatica, "Olig01/data/physeq_Olig01_all_metadata17.Rdata"))

# Get total counts per sample from the full phyloseq object
total_counts_Olig01_all <- data.frame(
  sample = sample_names(physeq_olig01_no_singletons),
  total_total_count = sample_sums(physeq_olig01_no_singletons)
)

# All (with unknown species)
# Merge combined diversity index table with total counts
combined_table_Olig01_INBO_with_total <- table_Olig01_annelida %>%
  dplyr::left_join(total_counts_Olig01_all, by = "sample")

# Save the updated combined table
write_to_csv_and_rdata(diversity_index_table = combined_table_Olig01_INBO_with_total,
                       output_directory = output_directory,
                       output_filename = "mbag_Olig01_INBO_dataframe_with_total_counts.csv")

# All (only known species)
# Merge combined diversity index table with total counts
combined_table_Olig01_INBO_with_total_known_species_tax_glom <- table_Olig01_annelida_known_species_tax_glom %>%
  dplyr::left_join(total_counts_Olig01_all, by = "sample")

combined_table_Olig01_INBO_with_total_known_species <- table_Olig01_annelida_known_species %>%
  dplyr::left_join(total_counts_Olig01_all, by = "sample")

# Save the updated combined table
write_to_csv_and_rdata(diversity_index_table = combined_table_Olig01_INBO_with_total_known_species_tax_glom,
                       output_directory = output_directory,
                       output_filename = "mbag_Olig01_INBO_dataframe_with_total_counts_known_species_tax_glom.csv")

write_to_csv_and_rdata(diversity_index_table = combined_table_Olig01_INBO_with_total_known_species,
                       output_directory = output_directory,
                       output_filename = "mbag_Olig01_INBO_dataframe_with_total_counts_known_species.csv")
...
# Load in the non-rarefied dataframe
physeq_ILVO_16S <- readRDS(file.path(file_directory_dataframe_overkoepelend, "16S_MBAG_with_metadata.rds"))
# Bring sample names to lower-case
sample_data(physeq_ILVO_16S)$Sample <- tolower(sample_data(physeq_ILVO_16S)$Sample)

# Make diversity table for 16S
table_ILVO_16S <- make_diversity_index_table_from_phyloseq_obj(
  phyloseq_obj = physeq_ILVO_16S,
  taxon_name = "Bacteria", unit_value ="ASV", primerset = "16S",
  metadata_df = metadata,
...
physeq_ILVO_16S_filtered <- readRDS(file.path(file_directory_data_ILVO_filtered, "Bacteria", "functional_guilds_ASVs", "MBAG_PS_16S_filtered.rds"))
# Bring sample names to lower-case (according to our choice in metadata)
sample_data(physeq_ILVO_16S_filtered)$Sample <- tolower(sample_data(physeq_ILVO_16S_filtered)$Sample)

# Make diversity table for
table_ILVO_16S_filtered <- make_diversity_index_table_from_phyloseq_obj(
  phyloseq_obj = physeq_ILVO_16S_filtered,
  taxon_name = "Bacteria_filtered", unit_value ="ASV", primerset = "16S",
  metadata_df = metadata,
  micro_or_macro_organism = "micro",
  tidytacos_sample_header = "Sample",
  list_runs_with_unrelated_samples = c("25002"),
...
# Combined the InseKP tables vertically
combined_table_16S_ilvo <- bind_rows(
  table_ILVO_16S,
  table_ILVO_16S_filtered
)

# Print to file the InseKP combined tables
write_to_csv_and_rdata(diversity_index_table = combined_table_16S_ilvo,
                       output_directory = output_directory,
                       output_filename = "mbag_16S_dataframe_ilvo.csv")


# Add total_total_count (all)

# Get total counts per sample from the full phyloseq object
total_counts_16S_ilvo_all <- data.frame(
  sample = sample_data(physeq_ILVO_16S)$Sample,
  total_total_count = sample_sums(physeq_ILVO_16S)
)

# All (with unknown species)
# Merge combined diversity index table with total counts
combined_table_16S_ILVO_with_total <- combined_table_16S_ilvo %>%
  dplyr::left_join(total_counts_16S_ilvo_all, by = "sample")

# Save the updated combined table
write_to_csv_and_rdata(diversity_index_table = combined_table_16S_ILVO_with_total,
                       output_directory = output_directory,
                       output_filename = "mbag_16S_ILVO_dataframe_with_total_counts.csv")


#---#
# List of bacterial phyla to subset
bacterial_16S_phyla <- c("Acidobacteriota", "Actinobacteriota", "Proteobacteria",
                         "Bacteroidota", "Nitrospirota", "Verrucomicrobiota", "Firmicutes")

# Create empty lists to store the subsets
ilvo_16S_subset <- list()
ilvo_16S_filtered_subsets <- list()

# Loop through each phylum and create subsets if present
for (phylum in bacterial_16S_phyla) {
  # Check for presence in original phyloseq object
  if (phylum %in% as.character(tax_table(physeq_ILVO_16S)[, "Phylum"])) {
    ilvo_16S_subset[[phylum]] <- subset_taxa(physeq_ILVO_16S, Phylum == phylum)
  }

  # Check for presence in filtered phyloseq object
  if (phylum %in% as.character(tax_table(physeq_ILVO_16S_filtered)[, "Phylum"])) {
    ilvo_16S_filtered_subsets[[phylum]] <- subset_taxa(physeq_ILVO_16S_filtered, Phylum == phylum)
  }
}
...
diversity_tables_16S_subset <- list()

# Loop through each phylum-specific phyloseq object
for (phylum in names(ilvo_16S_subset)) {
  phy_obj <- ilvo_16S_filtered_subsets[[phylum]]

  # Only proceed if the phyloseq object is not NULL (i.e., phylum was present)
  if (!is.null(phy_obj) && ntaxa(phy_obj) > 0) {
    diversity_tables_16S_subset[[phylum]] <- make_diversity_index_table_from_phyloseq_obj(
      phyloseq_obj = phy_obj,
      taxon_name = phylum,  # Dynamic taxon_name
      unit_value = "ASV",
      primerset = "16S",
      metadata_df = metadata,
      micro_or_macro_organism = "micro",
...
# ilvo_16S_filtered_subsets
# Create an empty list to store the resulting tables
diversity_tables_16S_filtered_subset <- list()

# Loop through each phylum-specific phyloseq object
for (phylum in names(ilvo_16S_filtered_subsets)) {
  phy_obj <- ilvo_16S_filtered_subsets[[phylum]]

  # Only proceed if the phyloseq object is not NULL (i.e., phylum was present)
  if (!is.null(phy_obj) && ntaxa(phy_obj) > 0) {
    diversity_tables_16S_filtered_subset[[phylum]] <- make_diversity_index_table_from_phyloseq_obj(
      phyloseq_obj = phy_obj,
      taxon_name = paste0(phylum, "_filtered"),  # Dynamic taxon_name
      unit_value = "ASV",
      primerset = "16S",
      metadata_df = metadata,
      micro_or_macro_organism = "micro",
...
# Combine all tables in each list (no extra columns added)
combined_diversity_16S_subset <- bind_rows(diversity_tables_16S_subset)
combined_diversity_16S_filtered_subset <- bind_rows(diversity_tables_16S_filtered_subset)

# Combine both into one final table
combined_all_diversity_tables_phyla_bacteria <- bind_rows(
  combined_diversity_16S_subset,
  combined_diversity_16S_filtered_subset
)
...
# Get total counts per sample from the full phyloseq object
total_counts_16S_ilvo_phyla_all <- data.frame(
  sample = sample_data(physeq_ILVO_16S)$Sample,
  total_total_count = sample_sums(physeq_ILVO_16S)
)
...
# All (with unknown species)
# Merge combined diversity index table with total counts
combined_table_16S_ILVO_phyla_with_total <- combined_all_diversity_tables_phyla_bacteria %>%
  dplyr::left_join(total_counts_16S_ilvo_phyla_all, by = "sample")

# observed is currently NA, should have observed = 0, because the data in this case is compositional
combined_table_16S_ILVO_phyla_with_total <- combined_table_16S_ILVO_phyla_with_total %>%
  mutate(
...
# Load in the non-rarefied dataframe
physeq_ILVO_Fungi <- readRDS(file.path(file_directory_dataframe_overkoepelend, "ITS2_MBAG_with_metadata.rds"))
# Bring sample names to lower-case
sample_data(physeq_ILVO_Fungi)$Sample <- tolower(sample_data(physeq_ILVO_Fungi)$Sample)

# Make diversity table for
table_ILVO_Fungi <- make_diversity_index_table_from_phyloseq_obj(
  phyloseq_obj = physeq_ILVO_Fungi,
  taxon_name = "Fungi", unit_value = "ASV", primerset = "ITS2",
  metadata_df = metadata,
  micro_or_macro_organism = "micro",
  tidytacos_sample_header = "Sample",
  list_runs_with_unrelated_samples = c("25002"),
...
# Load in the filtered dataframe
physeq_ILVO_Fungi_filtered <- readRDS(file.path(file_directory_data_ILVO_filtered, "Fungi", "functional_guilds_ASVs", "MBAG_PS_ITS2_filtered.rds"))
# Bring sample names to lower-case
sample_data(physeq_ILVO_Fungi_filtered)$Sample <- tolower(sample_data(physeq_ILVO_Fungi_filtered)$Sample)

# Make diversity table for
table_ILVO_Fungi_filtered <- make_diversity_index_table_from_phyloseq_obj(
  phyloseq_obj = physeq_ILVO_Fungi_filtered,
  taxon_name = "Fungi_filtered", unit_value = "ASV", primerset = "ITS2",
  metadata_df = metadata,
  micro_or_macro_organism = "micro",
  tidytacos_sample_header = "Sample",
  list_runs_with_unrelated_samples = c("25002"),
...
#---#
# List of fungi phyla to subset
fungi_phyla <- c("Basidiomycota", "Ascomycota", "Chytridiomycota",
                 "Mucoromycota", "Mortierellomycota")

# Create empty lists to store the subsets
ilvo_fungi_subset <- list()
ilvo_fungi_filtered_subsets <- list()

# Loop through each phylum and create subsets if present
for (phylum in fungi_phyla) {
  # Check for presence in original phyloseq object
  if (phylum %in% as.character(tax_table(physeq_ILVO_Fungi)[, "Phylum"])) {
    ilvo_fungi_subset[[phylum]] <- subset_taxa(physeq_ILVO_Fungi, Phylum == phylum)
  }

  # Check for presence in filtered phyloseq object
  if (phylum %in% as.character(tax_table(physeq_ILVO_Fungi_filtered)[, "Phylum"])) {
    ilvo_fungi_filtered_subsets[[phylum]] <- subset_taxa(physeq_ILVO_Fungi_filtered, Phylum == phylum)
  }
}
...
# ilvo_fungi_subset
# Create an empty list to store the resulting tables
diversity_tables_fungi_subset <- list()

# Loop through each phylum-specific phyloseq object
for (phylum in names(ilvo_fungi_subset)) {
  phy_obj <- ilvo_fungi_filtered_subsets[[phylum]]

  # Only proceed if the phyloseq object is not NULL (i.e., phylum was present)
  if (!is.null(phy_obj) && ntaxa(phy_obj) > 0) {
    diversity_tables_fungi_subset[[phylum]] <- make_diversity_index_table_from_phyloseq_obj(
      phyloseq_obj = phy_obj,
      taxon_name = phylum,  # Dynamic taxon_name
      unit_value = "ASV",
      primerset = "ITS2",
...
# ilvo_fungi_filtered_subsets
# Create an empty list to store the resulting tables
diversity_tables_fungi_filtered_subset <- list()

# Loop through each phylum-specific phyloseq object
for (phylum in names(ilvo_fungi_filtered_subsets)) {
  phy_obj <- ilvo_fungi_filtered_subsets[[phylum]]

  # Only proceed if the phyloseq object is not NULL (i.e., phylum was present)
  if (!is.null(phy_obj) && ntaxa(phy_obj) > 0) {
    diversity_tables_fungi_filtered_subset[[phylum]] <- make_diversity_index_table_from_phyloseq_obj(
      phyloseq_obj = phy_obj,
      taxon_name = paste0(phylum, "_filtered"),  # Dynamic taxon_name
      unit_value = "ASV",
      primerset = "ITS2",
...
# Combine all tables in each list (no extra columns added)
combined_diversity_fungi_subset <- bind_rows(diversity_tables_fungi_subset)
combined_diversity_fungi_filtered_subset <- bind_rows(diversity_tables_fungi_filtered_subset)

# Combine both into one final table
combined_all_diversity_tables_phyla_fungi <- bind_rows(
  combined_diversity_fungi_subset,
  combined_diversity_fungi_filtered_subset
)
...
# Get total counts per sample from the full phyloseq object
total_counts_fungi_ilvo_phyla_all <- data.frame(
  sample = sample_data(physeq_ILVO_Fungi)$Sample,
  total_total_count = sample_sums(physeq_ILVO_Fungi)
)
